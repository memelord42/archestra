...
export function PresetEditorDialog({
  ...
  cat: CatalogItem;
  /** When provided, dialog edits this preset. When null, creates new. */
  preset: Preset | null;
  /** When set (and preset is null), pre-fills the name field for "configure existing name" flows. */
  prefilledLabel?: string;
  open: boolean;
  onOpenChange: (v: boolean) => void;
}) {
  const { teams, upsertPreset, term } = useSpikeStore();
  ...
  // Reset form state when the dialog opens. Effect runs only on open/preset
  ...
  useEffect(() => {
    ...
    if (preset) {
      ...
      setVisibilityKind(preset.visibility.kind);
      if (preset.visibility.kind === "team")
        setTeamId(preset.visibility.teamId);
      setIsDefault(preset.isDefault);
      const next: FieldValues = {};
      for (const f of envFields) {
        const v = preset.fieldValues[f.key];
  ...
  function submit() {
    ...
    const fieldValues: Preset["fieldValues"] = {};
    for (const f of envFields) {
    ...
    const next: Preset = {
      id: preset?.id ?? `preset-${Math.random().toString(36).slice(2, 8)}`,
      catalogId: cat.id,
      label: trimmed,
      visibility:
        visibilityKind === "org"
      ...
      pinnedVersion: preset?.pinnedVersion ?? null,
    };
    upsertPreset(next);
    onOpenChange(false);
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[85vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-[560px]">
        <DialogHeader className="border-b px-6 py-4">
          <DialogTitle>
            {isEdit ? `Edit ${term.singular}` : `New ${term.singular}`}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
          ...
          {envFields.length > 0 && (
            <div className="space-y-3">
              <Label className="block">Field values</Label>
          ...
          <Label className="flex cursor-pointer items-center gap-2 rounded-md border border-border p-2.5 hover:bg-muted/30">
            ...
            />
            <span className="text-sm">Mark as default {term.singular}</span>
          </Label>
        </div>
